...
# 读取数据集文件 ct_relation.csv
df = pd.read_csv('./data/as_relation.csv')  # 根据实际情况选择正确的编码
...
# 将训练集和预测集保存为TSV文件
train_df.to_csv('as_train_set.tsv', sep='\t', index=False)
predict_df.to_csv('as_predict_set.tsv', sep='\t', index=False)
